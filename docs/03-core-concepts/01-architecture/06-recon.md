...
The db updates retrieved from OM is then converted into a batch of events for further processing by OM db tasks via [Recon Task Framework](#task-framework).
...
([/metrics](../../../2.0.0/interface/reconapi.html#metrics)) to query Prometheus. This integration can be enabled by setting this configuration `ozone.recon.prometheus.http.endpoint` to the Prometheus endpoint like `ozone.recon.prometheus.http.endpoint=http://prometheus:9090`.
...
- **ReconTaskStatus table**
  - Keeps track of the status and last run timestamp of the registered OM and SCM db tasks in the [Recon Task Framework](#task-framework)
...
- **UnhealthyContainers table**
  - Keeps track of all the Unhealthy Containers (MISSING, UNDER_REPLICATED, OVER_REPLICATED, MIS_REPLICATED) in the cluster at any given time
...
| `ozone.recon.sql.db.jdbc.url` | `jdbc:derby:$&#123;ozone.recon.db.dir&#125;`<br></br>`/ozone_recon_derby.db` | Recon SQL database jdbc url.                                                                                                         |
